...
def main(indicator, folder_indicator):
    ...
    #if False: #once it conver to fold*.csv, then it can be turn to FALSE
    ...
    if True: 
        for FOLD in tqdm(range(0, NUM_FOLDS)):
            ...
            model = load_model(glob.glob(f'{folder_indicator}/models/FOLD{FOLD}*')[0],compile=False, custom_objects={'Pearson': Pearson,'KendallTau': KendallTau})
            ####custom_objects:  Optional dictionary mapping names (strings) to custom functions to be considered during deserialization. 
            #####metrics=['accuracy','mean_absolute_error', Pearson,KendallTau] (from 3_Train.py: model was created with these customized metrics)            
